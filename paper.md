...
The threshold of the standard deviation filter is defined within Prerequiste (2). 

The analysis module has primary and secondary functionality. The primary analysis function is an iterative regression algorithm that defines a correlation profile between the atmospheric brightness temperature and the precipitable water vapor content. The relationship is characterized by a two-parameter exponential equation that takes the form:
\begin{equation}
\text{PW} = Ae^{T_b B}\, ,
\end{equation}
where $A, B$ are the best-fit coefficients, and $T_b$ is the atmospheric brightness temperature. This process results in two output files: One is for the results at each step, and the other is for the final averaged regression statistics. The plots associated with the analysis process will utilize the averaged coefficients and statistics. 
