...
modtran_plot <- function(xran, fname,offset, leg) {
    ...
    color = c("#00BCD7", "#FF9A00", "#66FF33", "#B426A5" ,"#D90000", "#005E8D")
    ...
    lines(fname[,1], rad(fname[,5]), cex=0.6, col=color[4])
    ...
    avg_3 <- avg_1 <- avg_2 <- avg_0 <- avg_4 <- list()
    for(i in 1:length(fname[,1])){
        if(fname[i,1] >= xran[1]){
            if(fname[i,1] <= xran[2]){
                ...
                avg_4 <- append(avg_4, rad(fname[i,5]))
            }
        }
    }
    ...
    avg4 <- Reduce("+", avg_4)/length(avg_4)
    ...
    curve(planck_curve(x, temp_planck_curve(avg0, avg4)), add=TRUE)

    points(avg0, avg1, pch=16, col=color[1])
    points(avg0, avg2, pch=16, col=color[2])
    points(avg0, avg3, pch=16, col=color[3])
    points(avg0, avg4, pch=16, col=color[4])

    curve(planck_curve(x, 240),col=color[5], add=TRUE)
    curve(planck_curve(x, 280),col=color[6], add=TRUE)

    points(avg0, Reduce("+", planck_curve(xran, 280))/length(planck_curve(xran, 280)), pch=16, col=color[6])
    points(avg0, Reduce("+", planck_curve(xran, 240))/length(planck_curve(xran, 240)), pch=16, col=color[5])

    text(8.8, 21.5, label="T = 280 K", col=color[6], srt=10)
    text(7.5, 5.85, label="T = 240 K", col=color[5], srt=10)
    ...
    text(9.5,offset[4], label=paste("T = ", round(temp_planck_curve(avg0, avg4),2), "K"), srt=5, cex=0.75)
...
modtran_plot2 <- function(xran, fname,offset, leg){
        rad <- function(data){
        intensity <- data * pi * 10^4
        return(intensity)
    }
    color = c("#00BCD7", "#FF9A00", "#66FF33","#D90000", "#005E8D")
    plot(fname[,1], rad(fname[,2]), type='n', xlab=NA, ylab=NA,
    main="Wavelength and Intensity Correlation", xlim=xran, ylim=c(0, 30))
    lines(fname[,1], rad(fname[,2]), cex=0.6, col=color[1])
    lines(fname[,1], rad(fname[,3]), cex=0.6, col=color[2])
...
modtran_plot(c(7,10), fname1, c(11.25,12.65,13.85,15), c("Water Vapor Scale = 0.5","Water Vapor Scale = 1.0","Water Vapor Scale = 1.5", "Water Vapor Scale = 2.0"))
modtran_plot2(c(7,10), fname2, c(11.5,12.75,14,12), c("-5 K Offset", "0 K Offset", "+5 K Offset"))
