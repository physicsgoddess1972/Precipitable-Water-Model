...
parser$add_argument("--src", help="Directory path to src folder")
parser$add_argument('--out', help="Directory path to out folder", default="./src/")
...
raw.dir <- 	file.path(args$dir)
out.dir <- 	file.path(args$out)
src.dir <- 	file.path(args$src)
...
err_war = yaml.load_file(file.path(sprintf("%spmat_codes.yml", src.dir)))
err = err_war$error
warn = err_war$warn

# Imports utility functions
source(sprintf("%spmat_utility.r", src.dir))

# Checks to see if _pmat.yml is present
# Throws error if the file is not in the expected directory
if(file.exists(file.path(raw.dir, "_pmat.yml"))){
	config	<- yaml.load_file(file.path(raw.dir, "_pmat.yml"))
...
# Throws error if the file is not in the expected directory
if (!file.exists(file.path(raw.dir, "cool_data.csv"))) {
...
if (!file.exists(file.path(raw.dir, "master_data.csv"))){
	logg("WARN",warn$f[[1]]$code, lev = level)
	logg("WARN",warn$f[[1]]$fix, lev = level)
	fname <- file.create(file.path(raw.dir, "master_data.csv"))
}
...
source(sprintf("%spmat_processing.r", src.dir))
...
# Analysis functions
source(sprintf("%spmat_analysis.r", src.dir))
# Visual and Data products functions
source(sprintf("%spmat_products.r", src.dir))
...
if (args$test){
	source("../tests/testthat/test_pmat_utility.r")
	source("../tests/testthat/test_pmat_processing.r")
	source("../tests/testthat/test_pmat_analysis.r")
