...
import logging, os, copy, warnings
...
class TFClassicalSVM():
    ...
    def postprocess(self, epoch, model, other_model, coeff, tr_size, batch_size):
        ...
        for i in range(epoch):
            ...
            train_acc_temp = sess.run(accuracy, feed_dict={x_data: x_vals_train, y_target: np.transpose(np.array(y_vals_train))})
            train_accuracy.append(train_acc_temp)

            test_acc_temp = sess.run(accuracy, feed_dict={x_data: x_vals_test, y_target: np.transpose(np.array(y_vals_test))})
    ...
    def evaluation(self, result, tr_size, epoch):
...
class ErrorTable():
    def errtab():
        ...
        table.add_row("Error (001)",  "RuntimeWarning: invalid value encountered in double_scalars\nmcc = cov_ytyp / np.sqrt(cov_ytyt * cov_ypyp)")
        table.add_row(" ", " ")
        table.add_row("Error (002)", "UndefinedMetricWarning: Precision is ill-defined and being set to 0.0 due to no predicted samples. Use `zero_division` parameter to control this behavior")
        table.add_row("---", "---")
            ...
            while j < 0.6:
                while True:
                    with tf.Session() as sess:
                        svm = TFClassicalSVM()
                        model, placeholder, coeff = svm.process(sess)
                        result = svm.postprocess(args.N, model, placeholder, coeff, i, 100)
                        warnings.filterwarnings('error')
                        try:
                            eval = svm.evaluation(result, i, args.N)
                            j = eval[2]
                            break
