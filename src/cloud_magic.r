...
data        <- read.csv(file="../data/master_data.csv", sep=",")
dummy_data  <- fastDummies::dummy_cols(data, select_columns = c("conditions"))

dummy_data$pw_avg      <- (dummy_data["PW_ABQ_12Z"] + dummy_data["PW_ABQ_00Z"] + dummy_data["PW_EPZ_12Z"] + dummy_data["PW_EPZ_00Z"])/4
dummy_data$temp_avg    <- (dummy_data["AMES_Air"] + dummy_data["FLIRi3_Air"])/2
