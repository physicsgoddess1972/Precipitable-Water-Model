...
#     print(list(typeof(iter), typeof(mean.out)))
    ...
    poster2 <- function(over.bool, iter, mean.out){
        ...
        ## Locational Averagen PW Temperature Correlation
            ...
            if (length(range) >= 2){
                for(j in 2:length(range)){
                    points(x, t(unlist(range[j])), pch=16, col=colscheme(length(range))[j])
                }
            }
            legend("topleft", legend=paste(unique(pw_time), "Z"), col=colscheme(length(range)), pch=c(16))
        ...
        ## Temporal Average Pw Temperature Correlation
            ...
            legend("topleft", legend=unique(pw_place), col=colscheme(length(range)), pch=16)
